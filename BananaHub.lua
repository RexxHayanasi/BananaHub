...
local Window = Rayfield:CreateWindow({
   Name = "🍌 banana Hub | fisch script 🐟",
   LoadingTitle = "🐟 fisch script",
   LoadingSubtitle = "by Airman",
   ConfigurationSaving = {
      Enabled = false,
   ...
   KeySettings = {
      Title = "Key | banana Hub",
      Subtitle = "banana Key",
      Note = "Key In Pastebin account AirmanGT",
      FileName = "Banana key", -- It is recommended to use something unique as other scripts using Rayfield may overwrite your key file
      SaveKey = false, -- The user's key will be saved, but if you change the key, they will be unable to use your script
      GrabKeyFromSite = true, -- If this is true, set Key below to the RAW site you would like Rayfield to get the key from
      Key = {"https://pastebin.com/raw/CzC8zmVR"} -- List of keys that will be accepted by the system, can be RAW file links (pastebin, github etc) or simple strings ("hello","key22")
   }
})
...
local Button = MainTab:CreateToggle({
 ...
 local args = {
        ...
        if rod then
            ...
            warn("Kings Rod not found in character.")
        end
    end
})
